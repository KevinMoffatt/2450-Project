...
for i = 1:rows
    ...
    deadWeight = ParameterMatrix(i,8);
    pa = ParameterMatrix(i,9);
    Patm = ParameterMatrix(i,10);
    Cd = ParameterMatrix(i,11);
    Cr = ParameterMatrix(i,12);
    muS = ParameterMatrix(i,13);
    Dw = ParameterMatrix(i,14);
    mw = ParameterMatrix(i,15);
    ...
    m = pt*volTrain + pa*Vol0 + mw + deadWeight;   %mass of train including deadwieght 
    g = 9.81;   %acceleration due to gravity (9.81 m/s)
    rp = Dp/2;  %radius of pinion = diameter of pinion/2
